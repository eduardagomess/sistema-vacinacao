...
class TelaPaciente():
    ...
    '''ef alterar_opcoes_paciente(self):
        layout =[
            [sg.Text('Editar Pacientes', size=(20, 1), font=("Helvetica", 15))],
            [sg.Radio('Editar nome', "AREA", key=1)],
            [sg.Radio('Editar telefone', "AREA", key=2)],
            [sg.Radio('Editar cpf', "AREA", key=3)], 
            [sg.Radio('Editar endereço', "AREA", key=4)],
            [sg.Radio('Editar data de nascimento', "AREA", key=5)],
            [sg.Button('Aplicar'), sg.Button('Sair')]
        ]'''
